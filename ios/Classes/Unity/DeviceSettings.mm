...
extern "C" int ParseDeviceGeneration(const char* model)
{
    ...
    else if (!strncmp(model, "iPhone12,1", 11))
        return deviceiPhone11;
    else if (!strncmp(model, "iPhone12,3", 11))
        return deviceiPhone11Pro;
    else if (!strncmp(model, "iPhone12,5", 11))
        return deviceiPhone11ProMax;
